...
const GENERIC = {
  what: 'A biomarker extracted from your health records. Tap to learn more about this specific marker.',
  why: 'Biomarkers provide objective data about your physiology, allowing evidence-based health optimization rather than guesswork.',
  high: 'Elevated values may indicate dysfunction, excess, or physiological stress. Review with your physician.',
  low: 'Below-range values suggest deficiency or reduced function. Causes vary — context and symptoms matter.',
  mitigateHigh: 'Lifestyle modifications (diet, exercise, sleep) and targeted supplementation are first-line. Medications as needed.',
  mitigateLow: 'Address root causes: nutrition, sleep, stress, exercise. Targeted supplementation or medication as appropriate.',
  goodFor: 'Optimal range supports normal physiological function and reduces disease risk.',
  badFor: 'Chronic imbalance in either direction increases risk of dysfunction and disease.',
};
...
const SECTION = ({ title, text, color = 'rgba(0,210,165,.65)' }: {title:string;text:string;color?:string}) => (
  <div style={{ marginBottom: 18 }}>
    ...
    <p style={{ fontSize: 14, color: 'rgba(0,210,165,.82)', lineHeight: 1.7, margin: 0 }}>{text}</p>
  </div>
);

export default function BiomarkerDetail({ marker, onClose }: Props) {
  const info = KNOWN[marker.name] || GENERIC;
      ...
      <div style={{ background: 'rgba(2,12,22,0.98)', border: '1px solid rgba(0,210,165,.2)', borderRadius: 12, padding: '32px 36px', maxWidth: 600, width: '90%', maxHeight: '85vh', overflowY: 'auto', position: 'relative' }}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            ...
            <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 24, color: 'rgba(0,220,175,.95)', margin: '0 0 8px', fontWeight: 400 }}>{marker.name}</h2>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'rgba(0,210,165,.4)', fontSize: 22, cursor: 'pointer', lineHeight: 1, padding: 4 }}>x</button>
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(0,210,165,.1)', paddingTop: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 18 }}>
            <div style={{ background: 'rgba(248,113,113,.06)', border: '1px solid rgba(248,113,113,.15)', borderRadius: 8, padding: '14px 16px' }}>
              <div style={{ fontSize: 11, color: 'rgba(248,113,113,.7)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>If too high</div>
              <p style={{ fontSize: 13, color: 'rgba(248,113,113,.8)', lineHeight: 1.65, margin: 0 }}>{info.high}</p>
            </div>
            <div style={{ background: 'rgba(251,146,60,.06)', border: '1px solid rgba(251,146,60,.15)', borderRadius: 8, padding: '14px 16px' }}>
              <div style={{ fontSize: 11, color: 'rgba(251,146,60,.7)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>If too low</div>
              <p style={{ fontSize: 13, color: 'rgba(251,146,60,.8)', lineHeight: 1.65, margin: 0 }}>{info.low}</p>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 18 }}>
            <div style={{ background: 'rgba(52,211,153,.04)', border: '1px solid rgba(52,211,153,.15)', borderRadius: 8, padding: '14px 16px' }}>
              <div style={{ fontSize: 11, color: 'rgba(52,211,153,.7)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Mitigate high</div>
              <p style={{ fontSize: 13, color: 'rgba(52,211,153,.8)', lineHeight: 1.65, margin: 0 }}>{info.mitigateHigh}</p>
            </div>
            <div style={{ background: 'rgba(56,189,248,.04)', border: '1px solid rgba(56,189,248,.15)', borderRadius: 8, padding: '14px 16px' }}>
              <div style={{ fontSize: 11, color: 'rgba(56,189,248,.7)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Mitigate low</div>
              <p style={{ fontSize: 13, color: 'rgba(56,189,248,.8)', lineHeight: 1.65, margin: 0 }}>{info.mitigateLow}</p>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={{ background: 'rgba(0,210,165,.04)', border: '1px solid rgba(0,210,165,.12)', borderRadius: 8, padding: '14px 16px' }}>
              <div style={{ fontSize: 11, color: 'rgba(0,210,165,.6)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Good for</div>
              <p style={{ fontSize: 13, color: 'rgba(0,210,165,.75)', lineHeight: 1.65, margin: 0 }}>{info.goodFor}</p>
            </div>
            <div style={{ background: 'rgba(167,139,250,.04)', border: '1px solid rgba(167,139,250,.15)', borderRadius: 8, padding: '14px 16px' }}>
              <div style={{ fontSize: 11, color: 'rgba(167,139,250,.7)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Watch out for</div>
              <p style={{ fontSize: 13, color: 'rgba(167,139,250,.8)', lineHeight: 1.65, margin: 0 }}>{info.badFor}</p>
            </div>
          </div>
        </div>

        <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid rgba(0,210,165,.08)', fontSize: 11, color: 'rgba(0,210,165,.3)', lineHeight: 1.5 }}>
