...
const SEV = {
  watch:    { color: '#f59e0b', border: 'rgba(245,158,11,.25)', bg: 'rgba(245,158,11,.05)', label: 'Worth watching' },
  concern:  { color: '#fb923c', border: 'rgba(251,146,60,.28)', bg: 'rgba(251,146,60,.06)', label: 'Needs attention' },
  critical: { color: '#f87171', border: 'rgba(248,113,113,.35)', bg: 'rgba(248,113,113,.07)', label: 'Act now' },
};
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ fontFamily: 'EB Garamond, Georgia, serif', fontSize: 22, color: 'rgba(220,255,235,.95)', marginBottom: 6 }}>Where are you heading?</div>
          <div style={{ fontSize: 13, color: 'rgba(0,210,165,.55)', lineHeight: 1.6, maxWidth: 520 }}>
      ...
      {markersWithHistory < 2 && !ran && (
        <div style={{ padding: '14px 16px', background: 'rgba(0,8,18,.4)', border: '1px solid rgba(0,210,165,.12)', borderRadius: 8, fontSize: 14, color: 'rgba(0,210,165,.55)', lineHeight: 1.7 }}>
      ...
      {loading && (
        <div style={{ padding: '20px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(0,210,165,.4)' }}>Analyzing marker trajectories, compliance patterns, and biological age slope</div>
        </div>
      )}
      ...
      {result && (
        <div>
          ...
          {result.overall_trajectory && (
            <div style={{ padding: '18px 20px', background: 'rgba(0,8,18,.6)', border: '1px solid rgba(0,210,165,.18)', borderLeft: '3px solid rgba(0,210,165,.5)', borderRadius: 8, marginBottom: 18 }}>
              <div style={{ fontSize: 11, color: 'rgba(0,210,165,.6)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>Overall Read</div>
              <p style={{ fontSize: 16, color: 'rgba(220,255,235,.95)', lineHeight: 1.8, margin: 0, fontWeight: 300 }}>{result.overall_trajectory}</p>
            </div>
          )}
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                            <div style={{ fontFamily: 'EB Garamond, Georgia, serif', fontSize: 18, color: 'rgba(220,255,235,.97)', fontWeight: 500 }}>{s.title}</div>
                            <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                              <span style={{ fontSize: 11, color: cfg.color, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{cfg.label}</span>
                              <span style={{ fontSize: 14, color: 'rgba(0,210,165,.35)', transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}>⌄</span>
                            </div>
                          </div>
                          <div style={{ fontSize: 12, color: 'rgba(0,210,165,.5)' }}>Timeline: {s.timeline}</div>
                        </div>
                      </div>
                      ...
                      {isOpen && (
                        <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${cfg.border}` }}>
                          <div style={{ marginBottom: 14 }}>
                            ...
                            <p style={{ fontSize: 15, color: 'rgba(220,255,235,.9)', lineHeight: 1.75, margin: 0, fontWeight: 300 }}>{s.what_happens}</p>
                          </div>
                          <div style={{ marginBottom: 14 }}>
                            <div style={{ fontSize: 11, color: '#f59e0b', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>You may already be feeling</div>
          ...
          <button onClick={() => { setRan(false); setResult(null); setExpanded(null); }}
            style={{ fontSize: 13, color: 'rgba(0,210,165,.5)', background: 'none', border: '1px solid rgba(0,210,165,.15)', borderRadius: 5, padding: '7px 14px', cursor: 'pointer', fontFamily: 'inherit' }}>
