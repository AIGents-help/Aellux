...
const SEVERITY_CONFIG = {
  watch:   { color: 'var(--accent-watch)', label: 'Worth watching',   bg: 'rgba(245,158,11,.06)',   border: 'rgba(245,158,11,.25)' },
  concern: { color: 'var(--accent-watch)', label: 'Needs attention',  bg: 'rgba(251,146,60,.06)',   border: 'rgba(251,146,60,.28)' },
  act:     { color: 'var(--accent-elevated)', label: 'Act on this',      bg: 'rgba(248,113,113,.07)',  border: 'rgba(248,113,113,.32)' },
};
...
export default function DoctorMissed({ document, allMarkers, userId, plan, profile }: Props) {
  ...
  return (
    <div style={{ marginTop: 16, padding: '14px 18px', background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: ran ? 14 : 0 }}>
        ...
        {!ran && !loading && (
          <button onClick={analyze}
            style={{ flexShrink: 0, fontSize: 13, padding: '7px 14px', background: 'var(--brand-ghost)', border: '1px solid var(--border-medium)', borderRadius: 5, color: 'var(--brand)', cursor: 'pointer', fontFamily: 'inherit', marginLeft: 12 }}>
            Analyze →
          </button>
        )}
      </div>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 14, color: 'var(--text-primary)', fontWeight: 500 }}>{flag.marker}</span>
                    ...
                    {isOpen && (
                      <div style={{ marginTop: 10 }}>
                        <p style={{ fontSize: 14, color: 'var(--text-primary)', lineHeight: 1.7, margin: '0 0 10px', fontWeight: 300 }}>
                          {flag.why_concerning}
                        </p>
                  ...
                  <span style={{ fontSize: 14, color: 'var(--text-tertiary)', flexShrink: 0, transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }}>⌄</span>
                </div>
              </div>
            );
          })}
