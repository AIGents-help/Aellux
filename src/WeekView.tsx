...
function MealRow({ slot, meal, dayIdx, selectedSwap, onSwap }: {
  ...
  return (
    <div style={{ marginBottom: 12, border: '1px solid var(--border-subtle)', borderRadius: 8, overflow: 'hidden', background: 'rgba(0,6,14,.4)' }}>
      <div
      ...
      >
        ...
        {/* Meal name + macros */}
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, color: 'var(--text-primary)', fontFamily: 'EB Garamond, Georgia, serif', lineHeight: 1.3 }}>
            {displayName}
            {!isOriginal && (
              <span style={{ fontSize: 11, marginLeft: 10, padding: '2px 8px', background: 'var(--border-subtle)', borderRadius: 3, color: 'var(--brand)', letterSpacing: '0.06em' }}>SWAPPED</span>
            )}
          </div>
          {meal.macros?.cal && (
        ...
        {isExpandable && (
          <div style={{ fontSize: 20, color: 'var(--text-tertiary)', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .2s', flexShrink: 0 }}>⌄</div>
        )}
      </div>

      {open && isExpandable && (
        <div style={{ padding: '12px 18px 18px', borderTop: '1px solid var(--brand-ghost)' }}>

          {/* Ingredients */}
          {meal.items && meal.items.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: 'var(--text-tertiary)', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8 }}>Ingredients</div>
              <div style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.7 }}>{meal.items.join(' · ')}</div>
            </div>
          )}
          ...
          {meal.flavor_boost && (
            <div style={{ marginBottom: 14, padding: '10px 14px', background: 'rgba(255,200,80,.06)', border: '1px solid rgba(255,200,80,.22)', borderRadius: 6 }}>
              ...
              <div style={{ fontSize: 15, color: 'var(--text-primary)', lineHeight: 1.6 }}>{meal.flavor_boost}</div>
            </div>
          )}

          {/* Why */}
          {meal.why && (
            <div style={{ fontSize: 14, color: 'var(--brand-dim)', fontStyle: 'italic', lineHeight: 1.65, marginBottom: 14, paddingLeft: 12, borderLeft: '2px solid var(--border-medium)' }}>
              {meal.why}
            </div>
          )}
          ...
          {hasAlts && (
            <>
              <div style={{ fontSize: 12, color: 'var(--text-tertiary)', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 10 }}>Swap to:</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 8 }}>
                {/* Original option */}
                <button
                  onClick={(e) => { e.stopPropagation(); onSwap(''); setOpen(false); }}
                  style={{ textAlign: 'left', padding: '10px 12px', background: isOriginal ? 'var(--border-subtle)' : 'var(--brand-ghost)', border: `1px solid ${isOriginal ? 'rgba(0,225,180,.55)' : 'var(--border-subtle)'}`, borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit', color: 'var(--text-primary)' }}
                >
                ...
                {meal.alternatives?.map((alt) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ textAlign: 'left', padding: '10px 12px', background: active ? 'var(--border-subtle)' : 'var(--brand-ghost)', border: `1px solid ${active ? SWAP_COLOR[alt.swap] : 'var(--border-subtle)'}`, borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit', color: 'var(--text-primary)' }}
                    >
...
function DayCard({ day, dayIdx, isToday, selected, onSwap }: {
  ...
  return (
    <div style={{ marginBottom: 16, border: `1px solid ${isToday ? 'rgba(0,225,180,.55)' : 'var(--border-subtle)'}`, borderRadius: 10, overflow: 'hidden', background: isToday ? 'rgba(0,32,50,.4)' : 'var(--bg-surface)' }}>
      ...
      <div onClick={() => setExpanded(!expanded)} style={{ padding: 'clamp(12px, 3vw, 18px) clamp(14px, 4vw, 22px)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
            <div style={{ fontFamily: 'EB Garamond, Georgia, serif', fontSize: 26, color: 'var(--text-primary)', fontWeight: 500 }}>{day.day}</div>
            {day.theme && (
        ...
        <div style={{ fontSize: 20, color: 'var(--text-tertiary)', transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform .2s', flexShrink: 0 }}>⌄</div>
      </div>

      {/* Expanded body */}
      {expanded && (
        <div style={{ padding: '0 clamp(12px, 4vw, 22px) clamp(14px, 3vw, 22px)', borderTop: '1px solid var(--border-subtle)' }}>
          ...
          {day.morning && (
            <div style={{ marginTop: 18 }}>
              ...
              {day.morning.actions?.map((a, i) => (
                <div key={i} style={{ fontSize: 15, color: 'var(--text-primary)', marginBottom: 5, lineHeight: 1.55 }}>• {a}</div>
              ))}
              {day.morning.supps_am && day.morning.supps_am.length > 0 && (
                <div style={{ fontSize: 14, color: 'var(--brand-dim)', marginTop: 8, lineHeight: 1.6 }}>
                  <strong style={{ color: 'var(--brand)' }}>AM supps:</strong> {day.morning.supps_am.join(', ')}
                </div>
              )}
            </div>
          )}
          ...
          {day.movement && (
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ fontSize: 16, color: 'var(--text-primary)', lineHeight: 1.5 }}>
                {day.movement.type}
          ...
          {day.evening && (
            <div style={{ marginTop: 20 }}>
              ...
              {day.evening.supps_pm && day.evening.supps_pm.length > 0 && (
                <div style={{ fontSize: 15, color: 'var(--text-primary)', marginBottom: 5, lineHeight: 1.6 }}>
                  <strong style={{ color: 'var(--brand)' }}>PM supps:</strong> {day.evening.supps_pm.join(', ')}
                </div>
              )}
              {day.evening.wind_down && (
                <div style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{day.evening.wind_down}</div>
              )}
            </div>
          )}
        </div>
...
export default function WeekView({ data, selectedMealKeys, onSwap, isPreview = false, onUpgrade }: Props) {
  ...
  return (
    <div>
      {/* Key insight */}
      {data.key_insight && (
        <div style={{ padding: '16px 20px', marginBottom: 16, background: 'var(--brand-ghost)', border: '1px solid rgba(0,225,180,.2)', borderRadius: 8 }}>
          <div style={{ fontSize: 12, letterSpacing: 1.5, textTransform: 'uppercase', color: 'rgba(0,225,180,.7)', marginBottom: 8 }}>Week design</div>
          <div style={{ fontSize: 16, color: 'var(--text-primary)', fontStyle: 'italic', lineHeight: 1.7 }}>{data.key_insight}</div>
        </div>
      )}

      {/* Principles */}
      {data.principles && data.principles.length > 0 && (
        <div style={{ padding: '14px 18px', marginBottom: 16, background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 8 }}>
          <div style={{ fontSize: 12, letterSpacing: 1.5, textTransform: 'uppercase', color: 'var(--text-secondary)', marginBottom: 10 }}>Guiding principles</div>
          {data.principles.map((p, i) => (
            <div key={i} style={{ fontSize: 15, color: 'var(--text-primary)', marginBottom: 6, lineHeight: 1.6 }}>• {p}</div>
          ))}
        </div>
      )}
      ...
      {isPreview && (
        <div style={{ padding: '24px 28px', marginTop: 20, background: 'rgba(0,8,18,.7)', border: '1px solid rgba(0,225,180,.45)', borderRadius: 10, textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: 'EB Garamond, Georgia, serif', fontSize: 22, color: 'var(--text-primary)', marginBottom: 10, lineHeight: 1.4 }}>Unlock the full 7-day Aellux Protocol</div>
          <div style={{ fontSize: 15, color: 'var(--brand-dim)', marginBottom: 20, lineHeight: 1.65 }}>Six more biologically distinct days, meal swaps, training rotation, supplement timing, and a printable weekly PDF.</div>
          <button onClick={onUpgrade} style={{ fontSize: 15, color: 'var(--brand)', background: 'rgba(0,195,155,.16)', border: '1px solid rgba(0,225,180,.55)', borderRadius: 6, padding: '12px 28px', cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '0.04em' }}>Upgrade to Aellux Pro — $29/mo →</button>
        </div>
      )}
      ...
      {data.weekly_summary && !isPreview && (
        <div style={{ padding: '16px 20px', marginTop: 16, background: 'rgba(255,200,80,.06)', border: '1px solid rgba(255,200,80,.25)', borderRadius: 8 }}>
          ...
          {data.weekly_summary.training_load && (
            <div style={{ fontSize: 15, color: 'var(--text-primary)', marginBottom: 5 }}><strong>Training:</strong> {data.weekly_summary.training_load}</div>
          )}
          {data.weekly_summary.total_supp_cost && (
            <div style={{ fontSize: 15, color: 'var(--text-primary)', marginBottom: 5 }}><strong>Supp cost:</strong> {data.weekly_summary.total_supp_cost}</div>
          )}
          {data.weekly_summary.estimated_calorie_target && (
            <div style={{ fontSize: 15, color: 'var(--text-primary)' }}><strong>Weekly calorie target:</strong> ~{data.weekly_summary.estimated_calorie_target.toLocaleString()}</div>
          )}
        </div>
      )}
    </div>
